...
namespace DEvTL.DiscordBot.Commands
{
    public class HelpModule : ModuleBase
    {
        ...
        public async Task Help(string path = "")
        {
            ...
            {
                var mod = _commands.Modules.FirstOrDefault(m => m.Name.ToLower() == path.ToLower());
                ...
                (mod.Submodules.Any() ? $"Submodules: {mod.Submodules.Select(m => m.Name.Normalize())}\n" : "") + " ";
                AddCommands(mod, ref output);
            }

            await ReplyAsync("", embed: output.WithInformationColor().Build());
        }
        ...
        public void AddHelp(ModuleInfo module, ref EmbedBuilder builder)
        {
            ...
            builder.AddField(f =>
            {
                f.Name = $"**{module.Name.Normalize()}**";
                f.Value = $"Submodules: {string.Join(", ", module.Submodules.Select(m => m.Name.Normalize()))}" +
                $"\n" +
